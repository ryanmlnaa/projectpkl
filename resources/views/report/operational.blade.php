    ...
    <div class="container-fluid mt-4">
        <div class="card shadow border-0">
            ...
            <div class="card-body">
                ...
                ========================================= --}}
                <form method="GET" action="{{ route('reports.operational.index') }}" class="row g-2 align-items-end mb-3">
                    <div class="col-md-3">
                        <label class="form-label">Filter Berdasarkan</label>
                        <select name="filter_field" class="form-control">
                            <option value="">-- Pilih Kolom --</option>
                            <option value="id_pelanggan" {{ ($filterField ?? '') === 'id_pelanggan' ? 'selected' : '' }}>ID
                                Pelanggan</option>
                            <option value="nama_pelanggan" {{ ($filterField ?? '') === 'nama_pelanggan' ? 'selected' : '' }}>
                                Nama Pelanggan</option>
                            <option value="bandwidth" {{ ($filterField ?? '') === 'bandwidth' ? 'selected' : '' }}>Bandwidth
                            </option>
                            ...
                            <option value="latitude" {{ ($filterField ?? '') === 'latitude' ? 'selected' : '' }}>Latitude
                            </option>
                            <option value="longitude" {{ ($filterField ?? '') === 'longitude' ? 'selected' : '' }}>Longitude
                            </option>
                            <option value="nomor_telepon" {{ ($filterField ?? '') === 'nomor_telepon' ? 'selected' : '' }}>
                                Nomor Telepon</option>
                            <option value="cluster" {{ ($filterField ?? '') === 'cluster' ? 'selected' : '' }}>Cluster
                            </option>
                        </select>
                    </div>
                    <div class="col-md-5">
                        <label class="form-label">Kata Kunci</label>
                        <input type="text" name="filter_query" value="{{ $filterQuery ?? '' }}" class="form-control"
                    ...
                    <div class="col-md-4">
                        ...
                        <a href="{{ route('reports.operational.index') }}" class="btn btn-outline-secondary">Reset</a>
                    </div>
                </form>
                            ...
                            @forelse($pelanggans as $i => $p)
                                <tr>
                                    ...
                                    <td class="d-flex gap-1">
                                        ...
                                        <form action="{{ route('reports.operational.update', $p->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf @method('PUT')
                                            ...
                                            <button type="submit" class="btn btn-sm btn-primary"
                                                title="Update cepat data terakhir">
                                                <i class="fas fa-save"></i>
                                            </button>
                                        </form>

                                        <form action="{{ route('reports.operational.destroy', $p->id) }}" method="POST"
